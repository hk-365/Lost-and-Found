...
const EditCard = ({ item, onClose, setItems }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!token) {
      ...
      await axios.put(`https://lost-and-found-ziq5.onrender.com/items/${item.id}`, { ...formData, image: imageUrl },
      { headers: { Authorization: `Bearer ${token}` } });
      const response = await axios.get('https://lost-and-found-ziq5.onrender.com/items');
